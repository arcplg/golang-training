                        ...
                        <div class="flex h-full flex-col justify-start gap-0 p-0">
                            <div class=" items-center grid grid-cols-12 gap-6">
                                <div class="col-span-9">
                                    <h4 class="font-semibold text-xl text-dark dark:text-white mb-3">Edit To do list</h4>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script setup lang="ts">
    ...
    const fetchToDo = async () => {
        try {
            ...
            const response = await axios.get(`http://localhost:8088/api/to-do-list/${id}`);
            console.log(response.data)

            toDo.value = response.data; // Gán dữ liệu API trả về cho todos
            console.log(toDo)
        } catch (error) {
    ...
    const updateToDo = async () => {
        try {
            ...
            const response = await axios.put(`http://localhost:8088/api/to-do-list/${id}`, toDo.value); // Use the correct Axios payload format
            console.log('To do edit:', response.data);

            alert('Updated successfully!');
            // Navigate to the home page after registration
            router.push('/homepage');
        } catch (error) {
            console.error('Error edit to do:', error.response?.data || error.message);
        }
    };

    onMounted(() => {
        fetchToDo(); // Gọi API khi trang được tải
    });
